...
class SafeKeys {
    ...
    async submitPass() {
        const p = this.$('pw').value.trim(); if (!p) return; this.$('pw').value = ''; this.showView('v-load');
    ...
    goToDash() { this.$('pg-auth').classList.remove('page--active'); this.$('pg-dash').classList.add('page--active'); this.renderPasswords(); this.renderNotes(); if(this.activeTab==='audit')this.renderAudit(); this.recalcStorage(); }
    async recalcStorage() { const r = await this.loadVaultFromDB(); if (r) this.updateStorageBar(new Blob([r]).size); }
    lock() { clearTimeout(this.autoLockTimer); this.key = null; this._currentPass = null; this.vault = []; this.notes = []; this.salt = null; this.closePanel(); this.$('pg-dash').classList.remove('page--active'); this.$('pg-auth').classList.add('page--active'); this.showView('v-menu'); this.refreshKeyBadge(); }
